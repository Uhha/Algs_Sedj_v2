 ...
 */
public class SuffixArrayX_mod {
    private static final int CUTOFF =  0;   // cutoff to insertion sort (any value between 0 and 12)
    ...
    private void insertion(int lo, int hi, int d) {
        for (int i = lo; i <= hi; i++)
            for (int j = i; j > lo && less(j, j-1, d); j--)  //!!  less(index[j], index[j-1], d); j--)
                exch(j, j-1);
    }
    ...
    private boolean less(int v, int w, int d) {
    	...
    	String dubS = text.toString()+text.toString();
...
//        
//        SuffixArrayX_mod suffix = new SuffixArrayX_mod(s);
//
